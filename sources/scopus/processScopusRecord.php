		...
		foreach($input as $field => $value)
		{
			if($field === 'coredata')
			{
				...
				foreach($value as $childField => $childValue)
				{
					...
					//in some cases the same childField may appear under a different parent field, or other changes need to be made to the childValue before it is saved
			...
			if($field === 'item')
			{
				...
				$correspondingAuthorsAddresses = array();
				foreach($input->xpath('//correspondence') as $correspondence)
				{
					...
					/* foreach($correspondence->affiliation as $affiliation)
					{
						$correspondingAuthorsAddresses[$name][] = (string)$affiliation->{'source-text'};
					} */
				}
				...
				foreach($input->xpath('//author-group') as $authorGroup)
				{
					...
					foreach($authorGroup->author as $author)
					{
						...
						if(in_array($authors[$seq]['value'], $correspondingAuthors))
						{
							...
							/* foreach($correspondingAuthorsAddresses[$authors[$seq]['value']] as $address)
							{
								$authors[$seq]['children']['irts.author.correspondingAddress'][]['value'] = $address;
							}
							
							unset($correspondingAuthorsAddresses[$authors[$seq]['value']]);*/
							unset($correspondingAuthors[$authors[$seq]['value']]); 
						}
				...
				//In many records funding-list -> funding-text duplicates the grantlist -> grant-text field, but not in all records, some have funding-text, but no grant-text
				if(!isset($acknowledgements))
				{
					foreach($input->xpath('//funding-list') as $fundingList)
					{
						if(isset($fundingList->{'funding-text'}))
						{
							$output['dc.description.sponsorship'][]['value'] = (string)$fundingList->{'funding-text'};
						}
					}
				}
