...
function harvestUnpaywall($source)
{
	...
	/* $records = getValues($irts, "SELECT idInSource handle, value doi FROM `metadata` 
		WHERE `source` = 'repository' 
		AND `field` = 'dc.identifier.doi' 
		AND `deleted` IS NULL
		AND `idInSource` IN (
			SELECT `idInSource` FROM `metadata` 
			WHERE `field` = 'dc.date.issued' 
		...
		AND `idInSource` IN (
			...
			AND `deleted` IS NULL 
			)", array('handle','doi'), 'arrayOfValues'); */
	...
	//Just check a few items to test
	//$records = array_slice($records, 0, 5);

	$hasFile = getValues($irts, "SELECT `idInSource` FROM metadata 
		WHERE `source` = 'repository' 
		AND `field` = 'dspace.bitstream.url' 
		AND deleted IS NULL", array('idInSource'), 'arrayOfValues');
	
	$recentlyChecked = getValues($irts, "SELECT `idInSource` FROM metadata 
		WHERE `source` = 'irts' 
		AND `field` = 'irts.check.unpaywall' 
		AND deleted IS NULL 
		AND `added` >= '".THREE_MONTHS_AGO."'", array('idInSource'), 'arrayOfValues');
		...
		if(!in_array($handle, $hasFile))
		{
			...
			if($check)
			{
				...
				if(is_string($responseJson))
				{
					...
					$result = saveSourceData($irts, 'unpaywall', $doi, $responseJson, 'JSON');
