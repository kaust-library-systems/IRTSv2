		...
		else
		{
			...
			if(isset($json['message']['items'][0]))
			{
				...
				if(isset($crossrefItem['title'][0]))
				{
					...
					{
						if(empty($firstCrossrefAuthorSurname))
						{
							$report .= ' -- No First Crossref Author Surname!'.PHP_EOL;
						}
						else
						{
			...
			{
				$report .= 'No Item in Message JSON of CrossRef Result!'.PHP_EOL;
			}
		}
		return $doi;
	}
