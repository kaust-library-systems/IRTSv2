<?php
	...
	function checkPerson($person)
	{
		...
		$fields = array(
			'localID'=>'local.person.id',
			'orcid'=>'dc.identifier.orcid',
			'scopusid'=>'dc.identifier.scopusAuthorID',
			'name'=>array('local.person.name','local.name.variant'),
			'controlName'=>'local.person.name',
			'email'=>'local.person.email',
			'kaustid'=>'local.person.id',
			'orgUnitNumber'=>'local.org.id',
