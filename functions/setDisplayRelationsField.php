...
function setDisplayRelationsField($record)
{
	...
	$relationsToDisplayInDspace = array(
		'dc.relation.issupplementto' => 'Is Supplement To', 
		'dc.relation.issupplementedby' => 'Is Supplemented By', 
		...
		'dc.relation.hasversion' => 'Has Version'
	);
	
	$prefixes = array(
		'DOI' => array('baseURL' => 'https://doi.org/', 'field' => 'dc.identifier.doi'), 
		'bioproject' => array('baseURL' => 'https://www.ncbi.nlm.nih.gov/bioproject/?term=', 'field' => 'dc.identifier.bioproject'), 
		'biosample' => array('baseURL' => 'https://www.ncbi.nlm.nih.gov/biosample/?term=', 'field' => 'dc.identifier.biosample'), 
			...
			foreach($record[$relationType] as $relatedIdentifier)
			{
				...
				foreach($prefixes as $prefix => $prefixData)
				{
					...
					if(is_array($relatedIdentifier))
						$relatedIdentifier = $relatedIdentifier['value'];
					
					if(strpos($relatedIdentifier, $prefix.':') !== FALSE)
					{	
						$relatedItem[$prefix] = str_replace($prefix.':', '', $relatedIdentifier);
						
						if(strpos($prefix, 'Handle') !== FALSE){
							$relatedItem[$prefix] = $prefixBaseURL.$relatedItem[$prefix];
						}
