...
// check if a DOI was entered in the form
if(isset($_POST['doi']) && preg_match('/10.\w*\/\w*/', $_POST['doi']))
{
	...
	if(identifyRegistrationAgencyForDOI($doi, $report)==='crossref')
	{
		...
		if(!empty($sourceData))
		{
			...
			$rowID = saveValue($_POST['source'], $_POST['idInSource'], 'dc.identifier.doi', 1, $doi, NULL);
...
else
{
	...
	$authors = getValues($irts, setSourceMetadataQuery($source, $idInSource, NULL, 'dc.contributor.author'), array('value'), 'arrayOfValues');
	
	$journal = getValues($irts, setSourceMetadataQuery($source, $idInSource, NULL, 'dc.identifier.journal'), array('value'), 'singleValue');

	$citation = $title.' '.implode('; ', $authors).' '.$journal.' '.getValues($irts, setSourceMetadataQuery($source, $idInSource, NULL, 'dc.date.issued'), array('value'), 'singleValue');
