<?php
	if(!isset($_POST['action']))
	{
		...
		//prepare empty record array
		$record = array();
		
		//prepare template for specific step, $step variable is also needed by snippets/displayForm.php
				...
				if(isset($_GET['year']))
				{
					$idInIRTS = getValues($irts, "SELECT `idInSource` FROM `metadata` 
						...
						AND idInSource IN (
							...
							WHERE `source` = 'irts' 
							AND `field` = 'dc.type' 
							AND value IN ('Article','Book Chapter','Conference Paper','Protocol')
							AND deleted IS NULL
						)
						AND idInSource IN (
							SELECT idInSource FROM `metadata` 
							...
							AND `deleted` IS NULL
							AND `value` NOT IN (
								SELECT tmp1.value FROM `metadata` tmp1 
									LEFT JOIN `metadata` tmp2 USING(idInSource)
									WHERE tmp1.`source` = 'repository' 
									AND tmp1.`field` = 'dc.identifier.doi' 
									AND tmp1.`deleted` IS NULL 
									AND tmp2.`source` = 'repository'
									AND tmp2.`field` = 'dspace.bitstream.url' 
									AND tmp2.`deleted` IS NULL
							)
						)
						AND deleted IS NULL 
						ORDER BY added ASC LIMIT $page, 1", array('idInSource'), 'singleValue');
				}
				else
				{
					$idInIRTS = getValues($irts, "SELECT `idInSource` FROM `metadata` WHERE `source` = 'irts' 
						AND `field` = 'irts.check.unpaywall' 
						AND value = 'inProcess' 
						AND idInSource IN ( 
							SELECT idInSource FROM `metadata` 
						...
						AND deleted IS NULL ORDER BY added ASC LIMIT $page, 1", array('idInSource'), 'singleValue');
				}				
			}
	...
	else
	{
		...
		{
			$page++;
			header("Location: reviewCenter.php?formType=$formType&itemType=$itemType&page=$page&year=$year");
			exit();
		}
		elseif($action === 'save')
		{
			...
			if($_GET['itemType']==='Acknowledgements')
			{
				...
				unset($record['kaust.acknowledgement.type']);
				unset($record['kaust.acknowledged.person']);
				...
				$record['dc.description.provenance'][] = 'Record metadata updated via REST API by '.$_SESSION['displayname'].' using the repository@kaust.edu.sa user account on '.TODAY.' as part of the acknowledgements review process.';

				$metadata = prepareItemMetadataAsDSpaceJSON($record);

				$response = putItemMetadataToDSpaceRESTAPI($itemID, $metadata, $DSpaceRESTAPIToken);
			...
			elseif($_GET['itemType']==='Unpaywall')
			{
				...
				else
				{
					...
					header("Location: reviewCenter.php?formType=$formType&itemType=$itemType&page=$page&year=$year&Message=SelectAFile");
					exit();
				}
