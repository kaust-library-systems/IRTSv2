<?php
	if(!isset($_POST['action']))
	{
		...
		{
			$unmatchedVariant = getValues($irts, "SELECT DISTINCT value FROM `metadata` WHERE `field` = 'kaust.acknowledged.supportUnit' AND value NOT IN (SELECT value FROM metadata WHERE source IN ('irts','local') AND field IN ('local.org.name','local.name.variant','irts.unmatched.orgUnitName')) ORDER BY value ASC LIMIT $page, 1", array('value'), 'singleValue');
		}
		...
		elseif($_GET['itemType']==='Person Name')
		{
			...
			$handles = getValues($irts, "SELECT idInSource FROM `metadata` WHERE source = 'repository' AND `field` = 'kaust.person' AND value = '$unmatchedVariant' AND deleted IS NULL", array('idInSource'), 'arrayOfValues');
		}
		...
		if($_GET['itemType']==='Person Name')
		{
			...
			'.$itemType.' To Match: <br><br><b>'.$unmatchedVariant.'</b><br><br>-- Listed as KAUST person on below publications:<br>';
			...
			<div class="col-lg-6">
				...
				foreach($listValues as $listValue)
				{
					...
					/* if(!isset($startMarked))
					{
						if(substr($listValue, 0, 3) === substr($unmatchedVariant, 0, 3))
						{
							$start = TRUE;
						}
					}
					
					if(!isset($start)||$startMarked)
					{
						echo '<option value="'.htmlspecialchars(json_encode($listValue, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_HEX_QUOT)).'">'.$listValue['value'].'</option>';
					}
					else
					{					
						echo '<option value="'.htmlspecialchars(json_encode($listValue, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_HEX_QUOT)).'" selected>'.$listValue['value'].'</option>';
		...
		else
		{
			...
			{
				echo '<button class="btn btn-block btn-danger" type="submit" name="action" value="external">-- Not a KAUST Unit or Address: Mark as External Affiliation --</button>
				<button class="btn btn-block btn-warning" type="submit" name="action" value="addressOnly">-- No Org Unit Match, Address Only: Add as Recognized Generic KAUST Address --</button>';
			}
			
			echo '<button class="btn btn-block btn-danger" type="submit" name="action" value="ignore">-- Ignore: Unable to Make a Confident Match --</button>
			<button class="btn btn-block btn-success" type="submit" name="action" value="matched">-- Matched: Match Selected From List --</button>
			<button class="btn btn-block btn-primary" type="submit" name="action" value="skip">-- Skip: Take No Action and Move to Next Item --</button>
			</form>
			</div>';
		}
	...
	else
	{
		...
		elseif($action === 'addressOnly')
		{
			...
			echo $unmatchedVariant.' successfully saved as generic KAUST address';
		...
		elseif($action === 'ignore')
		{
			if($_GET['itemType']==='Org Unit Name')
			{
				$currentPlace = getValues($irts, "SELECT `place` FROM metadata WHERE source = 'irts' AND idInSource = 'irts_unmatched' AND field = 'irts.unmatched.orgUnitName' AND deleted IS NULL ORDER BY place DESC LIMIT 1", array('place'), 'singleValue');
			
				$field = 'irts.unmatched.orgUnitName';
				$result = saveValue('irts', 'irts_unmatched', 'irts.unmatched.orgUnitName', $currentPlace+1, $unmatchedVariant, NULL);
				
				echo $unmatchedVariant.' will be ignored.';
			}
		...
		{
			$rowIDs = getValues($irts, "SELECT rowID FROM `metadata` WHERE source='irts' AND field='kaust.acknowledged.supportUnit' AND value = '$unmatchedVariant'", array('rowID'), 'arrayOfValues');
			
			foreach($rowIDs as $rowID)
			{
				update($irts, 'metadata', array("field"), array('kaust.acknowledged.person', $rowID), 'rowID');
			}
