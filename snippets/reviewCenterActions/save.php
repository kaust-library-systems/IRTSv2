<?php
	...
	else
	{
		...
		//get rowIDs of existing irts.note entries
		$rowIDs = getValues(
			$irts,
			"SELECT rowID FROM metadata WHERE source LIKE 'irts' AND idInSource LIKE '$idInIRTS' AND field LIKE 'irts.note' AND deleted IS NULL",
			array('rowID')
		);

		//mark existing problem notes as deleted
		markMatchedRowsAsDeleted($rowIDs, 'irts', $idInIRTS);
	}
	...
	$updateButton = '<form method="post" action="reviewCenter.php?'.$selections.'">
					...
					<button class="btn btn-lg btn-warning" type="submit" name="action" value="transfer">Update all metadata</button>
					</form>
					<form method="post" action="reviewCenter.php?'.$selections.'">
					<input type="hidden" name="page" value="'.($page).'">
					<input type="hidden" name="idInIRTS" value="'.$idInIRTS.'">
					<input type="hidden" name="handle" value="{handle}">
					<button class="btn btn-lg btn-primary" type="submit" name="action" value="createVersion">Create version</button>
					</form>';
